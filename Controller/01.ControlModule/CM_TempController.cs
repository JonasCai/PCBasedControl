...
public class CM_TempController : S88ControlModuleBase
{
    ...
    public void Stop()
    {
        _targetMode = TempControllerState.Disabled; // 任何时候都允许停止
    ...
    private void ChangeState(TempControllerState newState)
    {
        ...
        // 处理底层 PID/TimeProportioning 引擎的无扰切换与重置
        switch (newState)
        {
            case TempControllerState.Disabled:
    ...
    private void AlarmHandler()
    {
        ...
        if (AlarmState.HasAnyError)
        {
            ...
            _thisPidOutputPercent = 0f; // 瞬间清零，切断输出
        }
    ...
    private void RegisterCommandHandlers()
    {
        ...
        RegisterCommandHandler(Command.ChangeSP, cmd =>
        {
            ...
            else cmd.CallbackTcs?.TrySetResult(new CommandResult(CommandResultType.Rejected, "缺少设定值参数"));
        });
        ...
        RegisterCommandHandler(Command.ChangeManualOutput, cmd =>
        {
            ...
            else cmd.CallbackTcs?.TrySetResult(new CommandResult(CommandResultType.Rejected, "缺少手动输出参数"));
        });
